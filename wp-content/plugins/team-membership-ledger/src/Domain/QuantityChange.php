...
final class QuantityChange {
	 ...
	 *
	 * The unit price comes from the existing line (line total ÷ quantity), so a
	 * later change to the product's price does not re-price the charge.
	 *
	 * @return array{unit_price:float,new_total:float,is_paid:bool}
	 */
	public static function plan( int $old_qty, float $line_total, int $new_qty, float $paid ): array {
		if ( $old_qty < 1 ) {
